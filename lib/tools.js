...
const tools = {
  ...
  GetOpenEditors: {
    ...
    description:
      "Get metadata for all open text editors. Returns an array of {path, grammar, modified, lineCount, active}, empty when none are open. modified=true means the buffer holds edits the file on disk does not — read that file with ReadText rather than from disk, and do not overwrite it without telling the user.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  ReadText: {
    ...
    title: "Read Text",
    description:
      "Read an open editor's buffer, which is what the user is looking at. Reach for this when GetOpenEditors reports modified=true for a file: the buffer holds edits the file on disk does not, so your own file tools would read something the user has already changed. For a file with no unsaved changes, read it from disk instead. Modes: (1) offset/limit, which also returns hasMore. (2) start/end positions. (3) No params = whole buffer. Returns {content, path, totalLines, modified, range} in every mode, where range is {start: {row, column}, end: {row, column}} 0-indexed, or null when the path names no open editor and no editor is active.",
    inputSchema: {
      type: "object",
      properties: {
        path: {
