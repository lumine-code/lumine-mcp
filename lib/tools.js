...
const tools = {
  GetActiveEditor: {
    ...
    description:
      "Get active editor metadata. Returns {path, grammar, modified, lineCount}, or null when no text editor is active. Use ReadText for content, GetSelections for cursors/selections.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  GetOpenEditors: {
    ...
    description:
      "Get metadata for all open text editors. Returns an array of {path, grammar, modified, lineCount, active}, empty when none are open.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  ReadText: {
    ...
    title: "Read Text",
    description:
      "Read buffer content from active editor. For files >500 lines, use offset/limit pagination to avoid truncation. Modes: (1) offset/limit, which also returns hasMore. (2) start/end positions. (3) No params = full content (small files only). Returns {content, path, totalLines, range} in every mode, where range is {start: {row, column}, end: {row, column}} 0-indexed, or null when no text editor is active.",
    inputSchema: {
      type: "object",
      properties: {
        offset: {
  ...
  WriteText: {
    ...
    title: "Write Text",
    description:
      "Write text into active editor. With start: inserts at position (end defaults to start). With start+end: replaces range. Without: inserts at cursors. Returns {written: true, path} — plus oldText when a range was replaced — or {written: false} when no text editor is active.",
    inputSchema: {
      type: "object",
      properties: {
        text: {
