...
// The bridge's channel back to the host, held open for as long as the session
// lasts. The abort is what lets this process exit rather than wait on it.
...
/**
 * Listen on the bridge's own channel and relay what arrives.
 *
 * Everything else here is a reply to something the host asked; this is the one
 * direction that starts at the other end — today, the bridge saying its tool
 * list has moved because a package registered one or the user turned one off.
 */
async function openEventStream() {
  if (streamOpen || !target || !sessionId) return;
  streamOpen = true;
    ...
    for await (const chunk of response.body) {
      ...
      while ((boundary = buffer.indexOf("\n\n")) !== -1) {
        ...
        for (const line of frame.split("\n")) {
          // Anything else is a comment frame, and the bridge opens with one.
          if (!line.startsWith("data:")) continue;
          try {
            send(JSON.parse(line.slice(5).trim()));
          } catch {
            /* not a message we can relay */
          }
        }
      }
    }
  } catch {
    /* The editor went away. The next forward finds that out properly. */
  } finally {
    streamOpen = false;
    streamAbort = null;
  }
...
rl.on("line", async (line) => {
  ...
  try {
    ...
    // The session that initialize just opened is what the stream hangs off.
    if (sessionId && !streamOpen) openEventStream();
  } catch (error) {
...
rl.on("close", async () => {
  // Nothing is listening for what the stream carries any more, and an open
  // one would keep this process alive after the host has let go of it.
  streamAbort?.abort();
